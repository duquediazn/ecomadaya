(function () {
    ...
    * Crea el visor lightbox y devuelve una función para abrirlo con un conjunto de imágenes.
    ...
    /** CONSENTIMIENTO DE CONTENIDO EXTERNO (MEJORA PROGRESIVA) **/

    /**
     * Guarda cookie de consentimiento en cliente para evitar recarga cuando hay JS.
     * @param {string} name
     * @param {string} value
     * @param {number} maxAge
     * @returns {void}
     */
    const setClientConsentCookie = (name, value, maxAge) => {
    ...
    /**
     * Construye iframe externo a partir de los campos ocultos del formulario.
     * @param {HTMLFormElement} form
     * @returns {HTMLIFrameElement | null}
     */
    const buildIframeFromConsentForm = (form) => {
        const srcInput = form.querySelector('input[name="embed_src"]');
        if (!(srcInput instanceof HTMLInputElement) || !srcInput.value.trim()) {
            return null;
        }

        const titleInput = form.querySelector('input[name="embed_title"]');

        const embedWidth = form.dataset.embedWidth || '100%';
        const embedHeight = form.dataset.embedHeight || '350';

        const iframe = document.createElement('iframe');
        iframe.src = srcInput.value.trim();
        iframe.title = titleInput instanceof HTMLInputElement && titleInput.value.trim()
    ...
    /**
     * Intercepta formularios de consentimiento para cargar embeds sin recargar página cuando hay JS.
     * @returns {void}
     */
    const initExternalMediaConsentEnhancements = () => {
        document.querySelectorAll('[data-external-media-consent-form]').forEach((form) => {
            if (!(form instanceof HTMLFormElement)) {
                return;
            }

            form.addEventListener('submit', (event) => {
                const targetId = form.dataset.embedTarget || '';
                const targetContainer = document.getElementById(targetId);
                if (!(targetContainer instanceof HTMLElement)) {
                    return;
                }

                const iframe = buildIframeFromConsentForm(form);
                ...
                const cookieName = form.dataset.cookieName || 'madaya_external_media_consent';
                const maxAgeRaw = Number.parseInt(form.dataset.cookieMaxAge || '15552000', 10);
                const maxAge = Number.isFinite(maxAgeRaw) && maxAgeRaw > 0 ? maxAgeRaw : 15552000;
                setClientConsentCookie(cookieName, 'accepted', maxAge);
    ...
    const init = () => {
        ...
        initExternalMediaConsentEnhancements();
    };
